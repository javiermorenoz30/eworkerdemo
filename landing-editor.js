...
function blankValue(field) {
  ...
  if (field.type === 'array' || field.type === 'localizedArray') return []
  if (field.type === 'group') return Object.fromEntries((field.fields || []).map((child) => [child.key, blankValue(child)]))
  return ''
}
...
function injectStyles() {
  ...
  style.textContent = `
    ...
    .landing-editor-actions,.landing-section-tools,.landing-image-actions{display:flex;align-items:center;gap:8px;flex-wrap:wrap}.landing-editor-actions{justify-content:flex-end}
    ...
    .landing-editor-note{min-height:20px;margin:0;color:#66728d;font-size:12px}.landing-sections{display:grid;gap:12px}
    .landing-section-card{border:1px solid var(--line);border-radius:16px;background:#fff;overflow:hidden}.landing-section-card.is-dragging{opacity:.45}.landing-section-card.is-drop-target{outline:2px solid #5969d8;outline-offset:2px}
    .landing-section-summary{display:grid;grid-template-columns:auto minmax(0,1fr) auto;gap:13px;align-items:center;padding:16px 18px}.landing-drag{border:0;background:transparent;color:#8a95ad;font-size:18px;cursor:grab}
    ...
    .landing-mini{border:1px solid #dbe0ec;border-radius:8px;background:#fff;padding:7px 9px;color:#43516e;font-size:11px;font-weight:800;cursor:pointer}.landing-mini.danger{color:#a63d4e}
    .landing-section-menu{position:relative}.landing-section-menu summary{list-style:none}.landing-section-menu summary::-webkit-details-marker{display:none}.landing-menu-pop{display:none;position:absolute;right:0;z-index:5;min-width:145px;margin-top:6px;padding:7px;border:1px solid #dfe4ef;border-radius:10px;background:#fff;box-shadow:0 15px 35px rgba(34,48,82,.14)}.landing-section-menu[open] .landing-menu-pop{display:grid}.landing-menu-pop button{border:0;background:transparent;padding:9px;text-align:left;cursor:pointer}
    .landing-section-editor{border-top:1px solid var(--line);padding:18px;background:#fbfcff}.landing-section-editor[hidden]{display:none}.landing-language-tabs{display:flex;gap:7px;margin-bottom:16px}.landing-language-tabs button{border:1px solid #d9dfed;border-radius:999px;background:#fff;padding:8px 13px;color:#5f6b84;font-weight:800;cursor:pointer}.landing-language-tabs button.active{border-color:#5969d8;background:#eef0ff;color:#3342a5}
    ...
    .landing-fields,.landing-array,.landing-array-item,.landing-group{display:grid;gap:12px}.landing-field{display:grid;gap:7px}.landing-field>span,.landing-array-title{color:#56627d;font-size:11px;font-weight:800}.landing-field input,.landing-field textarea{width:100%;border:1px solid #d8deeb;border-radius:9px;background:#fff;padding:10px 11px;color:#17264d;font:inherit}.landing-field textarea{min-height:92px;resize:vertical}.landing-group,.landing-array-item{padding:13px;border:1px solid #e2e6f0;border-radius:11px;background:#fff}.landing-array-item-head{display:flex;justify-content:space-between;align-items:center;gap:10px}
    .landing-image-box{display:grid;grid-template-columns:120px minmax(0,1fr);gap:14px;align-items:center}.landing-image-preview,.landing-image-empty{width:120px;height:86px;border-radius:10px;border:1px solid #dfe4ee;background:#f1f3f8}.landing-image-preview{object-fit:cover}.landing-image-empty{display:grid;place-items:center;color:#8a94a9;font-size:11px}.landing-image-actions label{display:inline-flex;align-items:center;cursor:pointer}.landing-image-actions input[type=file]{position:absolute;width:1px;height:1px;opacity:0;pointer-events:none}
    .landing-add-row{display:flex;gap:9px;align-items:center;flex-wrap:wrap;padding:17px;border:1px dashed #ccd4e4;border-radius:14px;background:#fafbfe}.landing-add-row select{min-width:220px;border:1px solid #d8deeb;border-radius:9px;background:#fff;padding:10px}.landing-editor-legacy{display:none!important}
...
function createLocalizedField(section, path, label, { textarea = false } = {}) {
  ...
  for (const [locale, localeLabel] of [['es', 'Español'], ['en', 'English']]) {
    ...
    input.value = typeof value === 'object' && value !== null ? String(value[locale] ?? '') : locale === 'es' ? String(value ?? '') : ''
    input.dataset.sectionId = section.id
...
function createLinkField(section, path, label) {
  ...
  const caption = create('span', 'landing-array-title')
  caption.textContent = label
  group.append(caption, createLocalizedField(section, [...path, 'label'], 'Texto del enlace'), createTextField(section, [...path, 'href'], 'Destino'))
  return group
}
...
function createImageField(section, path, label) {
  ...
  if (imagePath) {
    const preview = create('img', 'landing-image-preview')
    preview.src = publicLandingImageUrl(imagePath)
    preview.alt = String(localizedValue(value.alt, 'es') || '')
    box.append(preview)
  } else {
  ...
  box.append(actions)
  group.append(caption, box, createLocalizedField(section, [...path, 'alt'], 'Descripción de la foto'))
  return group
}
...
function createArrayField(section, field, basePath = []) {
  ...
  values.forEach((_, index) => {
    ...
    item.append(head)
    field.itemFields.forEach((child) => item.append(createField(section, child, [...path, index])))
    group.append(item)
  })
...
function createLocalizedArrayField(section, field, basePath = []) {
  const path = [...basePath, field.key]
  const values = getAtPath(section.content, path) || []
  const group = create('div', 'landing-array')
  const title = create('span', 'landing-array-title')
  ...
  values.forEach((value, index) => {
    ...
    remove.type = 'button'
    remove.textContent = 'Eliminar'
    remove.setAttribute('data-localized-array-remove', 'true')
    remove.dataset.sectionId = section.id
    remove.dataset.localizedArrayPath = JSON.stringify(path)
    remove.dataset.localizedArrayIndex = String(index)
    head.append(label, remove)
    item.append(head)
    ...
    for (const [locale, localeLabel] of [['es', 'Español'], ['en', 'English']]) {
      ...
      wrapper.dataset.localizedLocale = locale
      const caption = create('span')
      caption.textContent = localeLabel
      const input = create('input')
      input.value = typeof value === 'object' && value !== null ? String(value[locale] ?? '') : locale === 'es' ? String(value ?? '') : ''
      input.dataset.sectionId = section.id
      input.dataset.localizedArrayPath = JSON.stringify(path)
      input.dataset.localizedArrayIndex = String(index)
      input.dataset.localizedArrayLocale = locale
      wrapper.append(caption, input)
      item.append(wrapper)
    }
    group.append(item)
  ...
  add.type = 'button'
  add.textContent = `+ Agregar ${field.label.toLowerCase()}`
  add.setAttribute('data-localized-array-add', 'true')
  add.dataset.sectionId = section.id
  add.dataset.localizedArrayPath = JSON.stringify(path)
  group.append(add)
  return group
}

function createGroupField(section, field, basePath = []) {
  const group = create('div', 'landing-group')
  const title = create('span', 'landing-array-title')
  title.textContent = field.label
  group.append(title)
  const nextPath = [...basePath, field.key]
  ;(field.fields || []).forEach((child) => group.append(createField(section, child, nextPath)))
  return group
}
...
function createField(section, field, basePath = []) {
  ...
  if (field.type === 'localizedArray') return createLocalizedArrayField(section, field, basePath)
  ...
  if (field.type === 'group') return createGroupField(section, field, basePath)
  return createTextField(section, path, field.label)
}

function sectionField(section, key, type) {
  return fieldsForSection(section).find((field) => field.key === key && (!type || field.type === type))
}
...
function ensureUniqueAnchor(section, allSections) {
  ...
  section.content.id = used.has(desired) ? `${desired}-${section.id.slice(0, 8)}` : desired
  return section
}
...
export async function initLandingEditor() {
  ...
  if (settingsGrid) settingsGrid.style.gridTemplateColumns = '1fr'
  ...
  shell.innerHTML = `
    <section class="landing-editor-head">
      <div><p class="eyebrow">PÁGINA PRINCIPAL</p><h2>Editar página principal</h2><p>Cambia textos, fotos y secciones. Nada se muestra al público hasta que pulses Publicar.</p></div>
    ...
    <div class="landing-add-row"><select id="landing-template-select" aria-label="Plantilla de nueva sección"></select><button type="button" class="primary" id="landing-add">+ Agregar sección</button></div>
  `
  ...
  async function cleanupMedia() {
    for (const path of [...orphanMedia]) {
      try {
        ...
        // Referenced files are protected by the API and can be retried later.
      }
    }
  }
  ...
  function renderSections() {
    ...
    sections.forEach((section, index) => {
      ...
      const menuButton = create('summary', 'landing-mini'); menuButton.textContent = '•••'
      const pop = create('div', 'landing-menu-pop')
      pop.append(makeButton('Duplicar', 'duplicate', ''), makeButton('Eliminar', 'delete', 'danger'))
      ...
      fieldsForSection(section).forEach((field) => fields.append(createField(section, field)))
  ...
  function mutateSection(action, sectionId) {
    ...
    if (!section) return
    if (action === 'edit') {
      openSections.has(sectionId) ? openSections.delete(sectionId) : openSections.add(sectionId)
      renderSections()
      return
    }
  ...
  list.addEventListener('click', (event) => {
    ...
    if (imageRemove) {
      ...
      markDirty(); renderSections(); return
    }

    const localizedAdd = event.target.closest('[data-localized-array-add]')
    if (localizedAdd) {
      const section = currentSection(localizedAdd.dataset.sectionId)
      if (!section) return
      const path = JSON.parse(localizedAdd.dataset.localizedArrayPath)
      const values = [...(getAtPath(section.content, path) || []), { es: '', en: '' }]
      setAtPath(section.content, path, values)
      markDirty(); renderSections(); return
    }

    const localizedRemove = event.target.closest('[data-localized-array-remove]')
    if (localizedRemove) {
      const section = currentSection(localizedRemove.dataset.sectionId)
      if (!section) return
      const path = JSON.parse(localizedRemove.dataset.localizedArrayPath)
      const values = [...(getAtPath(section.content, path) || [])]
      values.splice(Number(localizedRemove.dataset.localizedArrayIndex), 1)
      setAtPath(section.content, path, values)
      markDirty(); renderSections(); return
    }
    ...
    if (arrayAdd) {
      ...
      const field = section && sectionField(section, arrayAdd.dataset.arrayKey, 'array')
      if (!section || !field) return
      const values = [...(getAtPath(section.content, [field.key]) || []), blankArrayItem(field.itemFields)]
    ...
    if (arrayRemove) {
      ...
      const field = section && sectionField(section, arrayRemove.dataset.arrayKey, 'array')
      if (!section || !field) return
  ...
  list.addEventListener('input', (event) => {
    ...
    if (target.dataset.localizedArrayPath && target.dataset.localizedArrayLocale) {
      ...
      if (!section) return
      const path = JSON.parse(target.dataset.localizedArrayPath)
      const index = Number(target.dataset.localizedArrayIndex)
      const values = [...(getAtPath(section.content, path) || [])]
      const old = values[index]
      const item = old && typeof old === 'object' ? { ...old } : { es: String(old ?? ''), en: '' }
      item[target.dataset.localizedArrayLocale] = target.value
      values[index] = item
      setAtPath(section.content, path, values)
      markDirty()
      return
  ...
  previewButton.addEventListener('click', async () => {
    ...
    if (!ready) { previewWindow?.close(); return }
