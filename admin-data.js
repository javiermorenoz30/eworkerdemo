(() => {
  ...
  const officialInbox = 'eworker360@gmail.com'
  const defaults = {
    settings: {
  ...
  const load = () => {
    try {
      ...
      const state = saved ? { ...clone(defaults), ...saved, settings: { ...defaults.settings, ...saved.settings } } : clone(defaults)
      state.settings.contactEmail = officialInbox
      state.settings.notificationEmail = officialInbox
      return state
    } catch {
